...
export const HomeProvider = ({ children }) => {
  ...
  useEffect(() => {
    if(!currentPath.startsWith("/dashboard/item")) setOpen({
    type: null,
    focused: null,
  })
  }, [currentPath])
  ...
  useEffect(() => {
      ...
      const shouldLoadData = () => {
        if (!isAppReady || !currentPath.startsWith("/dashboard/item") || !open.type) {
          return false;
        }
        if (dataMap && dataMap[open.type] && dataMap[open.type].openData) return false;
        return true;
      };
  ...
  const updateFocusedPhaseCount = (newObj) => {
    if(newObj.status === open.focused?.status && newObj.phase === open.focused?.phase) setOpen((prev) => ({...prev, focused: null}))
