...
import { updatePageModifiersAction } from "@features/dashboard/utils/dashboardActions";
import { useInitializing } from "@core/context/InitializingContext";
import { useLocation, useNavigate } from "react-router-dom";
import useIsAdmin from "@shared/hooks/useIsAdmin";
import { fetchJobcostData, fetchJobcostItems } from "../api/jobcostApi";
...
export const JobcostProvider = ({ children }) => {
  ...
  const getBudget = (type) => {
    ...
    if (type === 1)
      return Number(finances.find((item) => item.Label === "Material").Value);
    if (type === 2)
      return Number(finances.find((item) => item.Label === "Labor").Value);
    if (type === 4)
      return Number(
        finances.find((item) => item.Label === "Subcontracts").Value,
      );
    if (type === 5)
      return Number(finances.find((item) => item.Label === "WTPM").Value);
  };
    ...
    const loadItems = async () => {
      ...
      const attemptLoad = async () => {
        ...
        try {
          ...
          const items = await fetchJobcostItems(
            mods,
            typeNum,
            controller.signal,
          );
