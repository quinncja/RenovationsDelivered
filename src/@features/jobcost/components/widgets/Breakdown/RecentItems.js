import { fileSvg } from "@assets/icons/svgs";
...
function RecentItems(props) {
    ...
    if (last && !item)
      return (
        <div
          ...
          style={{
            borderRadius: last ? "0px 0px 5px 5px" : "0px",
            marginBottom: last ? "10px" : "0px",
            opacity: "0",
          }}
        >
          <div
          ...
          >
            <h4 style={{ color: "var(--white)" }}> - </h4>
            <h5 style={{ fontStyle: "italic", opacity: 0.8 }}> - </h5>
          </div>
        </div>
      );
    return (
      ...
      >
        ...
        >
          {item.imagePath ? (
            <div className="tooltip-cube bigger-cube cube-svg">
              {" "}
              {fileSvg(getColor(item.id, "Tranquil"))}{" "}
            </div>
          ) : (
            <span
              className={`tooltip-cube bigger-cube ${item?.type === "committed" ? "committed-cube" : ""}`}
              style={{ backgroundColor: getColor(item.id, "Tranquil") }}
            ></span>
          )}
          <div
            style={{
        ...
        >
          <h4 style={{ color: "var(--white)" }}>
            {" "}
            {dollarFormatter(item.value)}{" "}
          </h4>
          <h5 style={{ fontStyle: "italic", opacity: 0.8 }}>
            {" "}
            {item.upddte
              ? dateTimeToString(dateFormatter(item.upddte))
              : dateTimeToString(dateFormatter(item.insdte))}{" "}
          </h5>
        </div>
      </div>
    );
  ...
  return (
    ...
    >
      <h4 style={{ textAlign: "left", paddingInline: "25px" }}>
        {" "}
        Recent Items{" "}
      </h4>
