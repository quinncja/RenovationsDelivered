import { fileSvg, tableArrow } from "@assets/icons/svgs";
...
function TableEntry({ headers, entry, currentId, nestingLevel = 0 }) {
  ...
  const getSubInfo = (obj) => {
    ...
    if (subId && obj[subId] && obj[subId].length > 0) {
      ...
      const remainingKeys = subKeys.filter(
        (key) => !subHeaderMap.hasOwnProperty(key),
      );
    ...
    return (
      ...
      >
        {subColumns.map(({ key, header }) => (
          <div className="sub-table-header-item" key={key}>
            {header}
          </div>
        ))}
      </div>
    ...
    return data.map((datum, index) => {
      return (
        <div className="sub-entry">
          {datum.imagePath && (
            <div
              className="table-entry-image-icon clickable-widget"
              onClick={(e) => handleOpen(e, datum)}
            >
          ...
          <div className="sub-entries" key={index}>
            {subColumns.map(({ key }) => (
              <div
                className="sub-table-entry-item"
                style={{ width: "50px" }}
                key={key}
              >
