...
function BarTooltip(datum, totalSum) {
  ...
  return (
    <div className="tooltip">
      <div style={{display: "flex", flexDirection: 'row', gap: '7px', alignItems: "center"}}> 
        <span className={`tooltip-cube ${type === "committed" ? "committed-cube" : ""}`} style={{ backgroundColor: datum.color }}></span>
        <h3> {vendor} </h3>
      </div>
      <div style={{display: "flex", flexDirection: 'column', gap: '1px', alignItems: "flex-start"}}>
      <h4 style={{color: "var(--white)"}}> <strong> {dollarFormatter(datum.value)} </strong> {type} </h4>
