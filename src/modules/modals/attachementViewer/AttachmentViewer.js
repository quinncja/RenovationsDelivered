import { close, fileSvg, leftArrowSvg, rightArrowSvg } from "business/svg";
import { useEffect, useState, useRef } from "react";
import { fetchFileFromSMB } from "utils/api";
import { formatSageUsername } from "utils/formatters";
    ...
    <div className="popup-wrapper">
      ...
      >
        <div style={{display: "flex", flexDirection: "column", width: "100%", height: "100%"}}>
          ...
          >
            {attachments.length > 1 && <button className="attachment-button" onClick={prevImage}>
              {leftArrowSvg()}
            </button>}
            {loading && !imageUrls[currentIndex] ? (
              <div style={{width: "100%",padding: "20px",  minHeight: "calc(100vh - 150px)"}} className="loading-widget" />
            ) : (
              <div style={{
                width: "100%", 
                display: "flex", 
                ...
                {renderFile(attachments[currentIndex].path, imageUrls[currentIndex])}
              </div>
            )}
            {attachments.length > 1 && <button className="attachment-button" onClick={nextImage}>
              {rightArrowSvg()}
            </button>}
          </div>
        </div>
