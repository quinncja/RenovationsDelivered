...
function NewChangeOrderModal(props){
    ...
    const inputText = {
      type: errorStates.type ? 
      "Please select CO type:" :
      "Select CO type:",
    ...
    const formValidator = () => {
      ...
      if (!modalData.pageModifiers && (!coModifiers.jobNum || !coModifiers.yearId || !coModifiers.phaseId)){
    ...
    const selectOrderType = () => {
      return(
        <>
        <strong style={{textAlign: "left", paddingLeft: "10px"}} className={`${descObj.error && "input-error-tag"}`}> {descObj.text } </strong>
    ...
    const selectModifers = () => {
      return(
      <>
        <strong style={{textAlign: "left", paddingLeft: "10px", paddingTop: "10px"}} className={`${modifiersObj.error && "input-error-tag"}`}> {modifiersObj.text } </strong>
        <div className="co-input-group" style={{ paddingBottom: "10px"}}>
          <ChangeOrderSelectors modifiers={coModifiers} handleModifierChange={handleModifierChange}/>
        </div>
    ...
    return (
        ...
        style={{alignSelf: "center", width: "25%", marginTop: "20px", padding: "15px"}}> 
        {loading ? 
        <div>
        <l-dot-pulse
          size="30"
