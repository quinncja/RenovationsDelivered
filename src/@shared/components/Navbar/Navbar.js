import { homeSvg, jobcostSvg, usersSvg } from "@assets/icons/svgs";
import Logo from "./Logo";
import { useLocation, useNavigate } from "react-router-dom";
import useIsAdmin from "@shared/hooks/useIsAdmin";
import { getBreakdownIconByType } from "@features/jobcost/utils/getters";
...
function NavButton({ obj, sub = false, height, last = false }) {
  ...
  const calculateNavItemHeight = (option) => {
    ...
    const totalHeight =
      baseHeight +
      subItemsGap +
      numSubItems * subItemHeight +
      (numSubItems - 1) * gapBetweenSubItems;
