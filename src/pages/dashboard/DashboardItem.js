...
const DashboardItem = memo(forwardRef((props, ref) => {
  ...
  const dataToShow = dragging ? [] : showSingle ? chartObj.single.cleaner(data) : data
  ...
  const { attributes, listeners, transform, transition, setNodeRef } = useSortable({
    ...
    data: {
      ...
      renderDragOverlay: () => (
        <DraggingItem children={children} chartType={chartType} type={type} id={id} chart={chartToShow} data={dataToShow} />
      ),
    },
  });
    ...
    >
      ...
      <div className="widget-top">
        ...
        <AnimatePresence>
          ...
          >
            <ChartDisplay chartObj={chartToShow} data={dataToShow} id={id}/>
          </motion.div>
          </AnimatePresence>
        }
      </motion.div>
