...
function DashboardOpen(){
    ...
    const renderBody = () => {
        if(type === "data-validation") return <DataValidationPage {...openData}  id={currentType}/>
        else if(type === "phase-overview") return <PhaseCountPage {...openData}/>
        else if(type && type.includes("insight")) return <InsightPage {...openData}/>
        else return ""
    }
    const insightTypes =  ["client", "project", "vendor", "subcontractor"]
    const dataValidationTypes = ["critical-issues", "data-quality", "data-integrity"]
    ...
    return(
        <div className="job-cost-dashboard">
                ...
                <h2> {normal} {currentType ? `• ${kebabToNormal(currentType)}` : ""}</h2>
