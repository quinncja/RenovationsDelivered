...
function PhaseCountOpenChart({ data, focused }) {
  const year = new Date().getFullYear();
  const lastYear = year - 1;
  const { updateFocusedPhaseCount } = useHome();
  ...
  const phaseData = data
    ...
    .reduce((acc, item) => {
      ...
      const normalizedPhase = (phaseNum === 0 || phaseNum > 12) ? "13" : item.phase;
      ...
      if (existingIndex >= 0) {
        ...
        acc[existingIndex].previous_year_total = acc[existingIndex].previous_year_open + acc[existingIndex].previous_year_closed;
        acc[existingIndex].total = acc[existingIndex].current_year_open + acc[existingIndex].current_year_closed;
      } else {
        acc.push({
          ...
          previous_year_total: (item.previous_year_open || 0) + (item.previous_year_closed || 0),
  ...
  const LineLayer = ({ bars, xScale, yScale, innerHeight, innerWidth }) => {
    const lineData = phaseData.map((d, i) => {
  ...
  const getColor = (datum) => {
    if(!focused) return datum.id === 'current_year_open' ? 'var(--open)' : 'var(--closed)';
    else if(focused.textStatus === datum.id.split('_')[2] && focused.phase === datum.data.phaseNum) return datum.id === 'current_year_open' ? 'var(--open)' : 'var(--closed)';
    else return '#212831'
  }
...
export default PhaseCountOpenChart;