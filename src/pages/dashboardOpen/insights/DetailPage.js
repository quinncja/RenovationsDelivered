...
import { destructureJobNumber, dollarFormatter, getBaseJobName, strToMods } from "utils/formatters";
...
function DetailPage({ type, detail, detailData }) {
    ...
    if(!detailData) return(
      <>  
        ...
        <div className="detail-header" style={{height: "200px", width: "100%"}}>
          <div className="loading-widget"/>
        </div>
      </>
    )
      ...
      return(
        <div className="detail-header" style={{display: 'flex', justifyContent: "space-between", alignItems: 'center'}}>
          <div style={{display: 'flex', flexDirection: "row", gap: "15px", alignItems: "center"}}> 
            <div style={{height: '25px', width: "25px", borderRadius: "5px", background: color}}/>
          ...
          <div style={{display: 'flex', flexDirection: "column", textAlign: "right", gap: "5px"}}>
    ...
    const renderItem = (item) => {
      ...
      return (
          <div className={`phase-body aging-item ${canClick && "clickable-widget"}`} onClick={handleClick}>
            <div style={{
              display: "grid",
      ...
      const renderSortableHeaders = () => {
        return (
          <div className="phase-body aging-item" style={{padding: "0px", backgroundColor: "var(--terciary)"}}>
            <div 
              style={{
                display: "flex",
                alignItems: "center",
            ...
            >
              <h4 style={{fontWeight: "600", color: "white"}}> {detailData.items.length} {detailData.items.length === 1 ? type === "project-insight" ? "Phase" : "Project" : type === "project-insight" ? "Phases" : "Projects"}</h4>
            </div>
            <div 
            ...
            >
              ...
              {sortSvg(sortConfig.key === 'contract_amount' ? sortConfig.direction : null)}
            </div>
          </div>
        )
      }
    ...
    return(
      ...
      <div style={{width: '100%', boxSizing: "border-box"}}>
        {detailData.items && detailData.items.length > 0 && renderSortableHeaders()}
        {detailData.items ? (
