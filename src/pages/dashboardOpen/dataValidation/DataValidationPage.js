...
function DataValidationPage({ data, id }){
  ...
  const renderHeader = () => {
    if(!hasLoaded) return(
      <div className="validation-header" style={{height: "101px", boxSizing: "border-box"}}> 
        <div className="loading-widget"/> 
      </div>
    )
    return(
      <div className="validation-header ">
        <div className="dv-card-header" style={{marginBottom: "0px", alignItems: "center"}}>
          <div className={`status-indicator status-indicator-bigger ${obj.className}`}></div>
          <div style={{display: "flex", flexDirection: "column"}}>
            <div className="dv-card-value" style={{marginTop: "0px", fontSize: "42px"}}>{widgetData[0][obj.accessor]}</div>
            <h4> {obj.subtitle} </h4>
          </div>
        </div>
      </div>
  ...
  const items = homeData ? homeData.filter((item) => item.category === obj.accessor) : null;
  ...
  const renderItem = (item) => {
    ...
    return(
      <div className="validation-item">
        ...
        >
          <h3> {getBaseJobName(item.jobnme)} <span style={{fontWeight: "500", fontColor: "var(--secondary-font)", fontSize: "14px", opacity: .8}}> P{phase} {year} </span> </h3>
          <h4> {item.JobNumber} </h4>
        </div>
        <div className="left-border" style={{display: "flex", flexDirection: 'column', height: "100%", justifyContent: "center"}}>
          <h5> Details </h5>
  ...
  const renderItems = (items) => {
    if(!items) return (
      <div className="validation-body" style={{display: 'flex', flexDirection: "column", width: "100%", height: "150px"}}>
