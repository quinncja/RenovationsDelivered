...
import { downArrowSvg, rightArrowSvg } from "business/svg";

function JobList(){
  const { pageModifiers, updatePageModifiers } = useJobCostContext();
  const { getJobListByPageModifiers, phaseListToProjectList } = useProjectContext();
  ...
  const handlePhaseClick = useCallback((e, phase) => {
    e.stopPropagation();
    const mods = {
      jobNum: phase.jobNum,
      yearId: phase.yearId,
  ...
  const handlePhaseToggle = useCallback((e, projectJobNum) => {
    e.stopPropagation();
    setExpandedProject(expandedProject === projectJobNum ? null : projectJobNum);
  }, [expandedProject]);
    
  const renderPhase = useCallback((phase) => {
    return(
      <button 
        key={phase.id} 
  ...
  const renderPhaseCount = useCallback((project) => {
  ...
  const renderProject = useCallback((project) => {
    ...
    return(
      <div 
        key={project.jobNum}
        className="project-item recent-item"
      >
    ...
    return (
      <div className="pagination">
        ...
        <span className="pagination-info">
          {startIndex + 1}-{Math.min(endIndex, totalProjects)} of {totalProjects}
        </span>
  ...
  }, [totalPages, currentPage, handlePageChange, startIndex, endIndex, totalProjects]);
