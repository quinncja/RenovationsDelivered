...
function Breakdown(props) {
  ...
  if (!budget && !spent)
    return (
      ...
      <div className="breakdown-widget-header">
        <div style={{display: 'flex', flexDirection: "row", gap: "15px", alignItems: "center"}}> 
          <div className={`breakdown-icon-box breakdown-icon-box-white`}> {icon} </div>
          <h2 className="breakdown-title"> {type} </h2>
        </div>
      </div>
        <EmptyBreakdown/>
  ...
  return (
    <div className="breakdown-widget clickable-widget" onClick={() => openBreakdownPage(type, null)}>
      <div className="breakdown-widget-header">
        <div style={{display: 'flex', flexDirection: "row", gap: "15px", alignItems: "center"}}> 
          <div className={`breakdown-icon-box breakdown-icon-box-white`} style={{background: background}}> {icon} </div>
          <h2 className="breakdown-title"> {type} </h2>
        </div>
      </div>
      <motion.div
        ...
        animate="visible"
        exit="exit"
        style={{ position: "relative",  flex: 1,  minHeight: 0, gap: "5px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}
        variants={itemFadeIn}
      >
        <CostBreakdown budget={budget} costItems={costItems} color={color} type={type} />
        {costItems.posted.length === 0 && costItems.committed.length === 0 ? 
          <EmptyBreakdown/> :
          <>
          <div style={{ display: "flex", flex: 1, minHeight: 0, gap: "0px", paddingTop: "0px"}}>
