...
function OpenBreakdown(){
    const { breakdownData, pageModifiers, openBreakdownPage } = useJobCostContext();
    ...
    return(
        <div className="job-cost-dashboard">
            ...
            <h2> {type} {isVisible2 ? "Breakdown" : "Cost Items"} {!isVisible && ` - ${pageModifierToString(pageModifiers)}`} </h2>
                <div style={{display: 'flex', gap: "20px", paddingRight: "0px"}}> 
                {getOtherTypes(type).map((type) => 
                <button className="jobs-header-btn" onClick={() => openBreakdownPage(type)}>
