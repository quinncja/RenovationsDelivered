...
import { calculateTotalSum, getBreakdownIconByType, getIconBackground, getStatusColor } from "utils/funcs";
...
function OpenCostBreakdown(props){
    ...
    const infoBoxes = () => {
            return(
              <div style={{display: "flex", flexDirection: "row", width: "100%", flexGrow: "1", alignItems: "flex-end", boxSizing: "border-box", justifyContent: "flex-start", gap: "0px", overflowX: "scroll", paddingInline: "25px"}}>
                <div style={{display: "flex", flexDirection: "column", gap: '10px', paddingRight: "40px" }}> 
                  <div style={{display: 'flex', flexDirection: "row", gap: "10px", alignItems: "center"}}> 
                    <div className={`breakdown-icon-box breakdown-icon-box-bigger breakdown-icon-box-white}`} style={{background: background}}> {icon} </div>
                      <div className="" style={{display: 'flex', flexDirection: "column", justifyContent: "flex-start", textAlign: 'left', flexShrink: 0}}>
                        <h4>
                            {type} Budget
                        </h4>
                        <h2 style={{fontSize: '24px'}}>
                            {dollarFormatter(budget)}
                        </h2>
                      </div>
                  </div>
                  <div style={{display: 'flex', alignItems: 'center', gap: '10px', border: `.5px solid ${color}`, background: remainder > 0 ? "rgba(40, 167, 69, 0.1)" : "rgba(255, 71, 87, 0.1)", padding: '8px', paddingLeft: "10px", borderRadius: '5px'}}> 
                    <div style={{width: "5px", height: '5px', borderRadius: "10px", backgroundColor: color}} />
                    <h4 style={{fontSize: "10px", fontWeight: "600", color: color, textTransform: "uppercase"}}>
                      {budgetText}
                    </h4>
                  </div>
                </div>

                <div className="jobcost-detail-box" style={{display: 'flex', flexDirection: "column", textAlign: 'left'}}>
                    <h4>
                        Total Spent
                    </h4>
                    <div style={{display: "flex", flexDirection: 'column', gap: "2px"}}> 
                    <h2 style={{fontSize: '22px'}}>
                        {dollarFormatter(spent)}
                    </h2>
                ...
                <div className="jobcost-detail-box" style={{display: 'flex', flexDirection: "column", textAlign: 'left'}}>
                    ...
                    <h5 style={{opacity: isFinite(remainingPercent) ? 1 : 0, color: color}}>
    ...
    return (
        <div className="open-breakdown-cost">
            ...
            <div className="breakdown-progress-bar open-progress-bar" style={{ position: "relative" }}>
              {spent > budget ? (
                <>
                  {postedSum > 0 && (
                    <div
                      style={{
                        height: "12px",
                        borderRadius: actualCommittedAmount > 0 ? "0px 0px 0px 5px" : "0px 0px 0px 5px",
                      ...
                      onMouseEnter={() => handleMouseEnter("Invoiced", postedSum, postedPercent)}
                  ...
                  {actualCommittedAmount > 0 && (
                    <div
                      style={{
                        height: "12px",
                        borderRadius: postedSum > 0 ? "0px 0px 5px 0px" : "0px 0px 0px 5px",
                      ...
                      onMouseEnter={() => handleMouseEnter("Committed", actualCommittedAmount, actualCommittedPercent)}
                      onMouseLeave={handleMouseLeave}
                    />
                  )}
                ...
                <>
                  {postedSum > 0 && (
                    <div
                      style={{
                        height: "12px",
                        borderRadius: actualCommittedAmount > 0 ? "0px 0px 0px 5px" : "0px 15px 15px 5px",
                  ...
                  {actualCommittedAmount > 0 && (
                    <div
                      style={{
                        height: "12px",
                        borderRadius: postedSum > 0 ? "0px 15px 15px 0px" : "0px 15px 15px 5px",
