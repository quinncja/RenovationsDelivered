...
import { dollarFormatter, formatNumberShort, phaseNumToMonth, phaseToShortMonth } from "utils/formatters";

function YearRevenueLineChart({ data }) {
  ...
  const CustomSliceTooltip = ({ slice }) => {
    ...
    if (currentYearPoint && lastYearPoint) {
      ...
      const cumulativeGrowthPercent = ((currentCumulative - previousCumulative) / previousCumulative) * 100;
      
      const currentMonthly = currentYearPoint.data.monthlyRevenue;
      const previousMonthly = lastYearPoint.data.monthlyRevenue;
      const monthlyGrowthPercent = ((currentMonthly - previousMonthly) / previousMonthly) * 100;
      
      cumulativeGrowth = cumulativeGrowthPercent;
      monthlyGrowth = monthlyGrowthPercent;
      growthColor = cumulativeGrowthPercent >= 0 ? 'var(--green)' : 'var(--red)';
    }
    
    return (
      <div className='tooltip' style={{ minWidth: "220px" }}>
        <h4>
          {phaseNumToMonth(points[0].data.month)}
        </h4>
              ...
              <div style={{ 
                ...
                <div style={{ 
                  ...
                  <h4 style={{ 
                    ...
                    {cumulativeGrowth > 0 ? '+' : ''}{cumulativeGrowth.toFixed(1)}%
                  </h4>
                </div>
              </div>
              ...
              {monthlyGrowth !== null && (
                <div style={{ 
                  ...
                  <div style={{ 
                    ...
                    <h4 style={{ color: monthlyGrowth >= 0 ? 'var(--green)' : 'var(--red)'}}>
                      {monthlyGrowth >= 0 ? '↗' : '↘'}
                    </h4>
                    <h4 style={{ 
                      color: monthlyGrowth >= 0 ? 'var(--green)' : 'var(--red)', 
                    }}>
    ...
    <>
      <ResponsiveLine
        ...
        pointBorderColor={(point) => {
          ...
          const index = chartData.findIndex(series => series.id === point.serieId);
...
export default YearRevenueLineChart;