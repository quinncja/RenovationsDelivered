...
function EmployeePerformance(){
    ...
    const employeeCard = (employee) => {
        const {firstName, lastName, employeeNum, margin, totalContract} = employee;

        return(
            <div className="widget clickable-widget"
                id={employeeNum}
                onClick={() => handleClick(employeeNum)}
                style={{display: "flex", flexDirection: "column", justifyContent: "space-between", paddingInline: '20px', gap: "20px", boxSizing: "border-box"}}> 
                    <div style={{display: "flex", flexDirection: "row", gap: "10px", alignItems: "center"}}>
                        <div className='tjh-box' style={{borderRadius: "50%", flexShrink: 0, color: "white", fontWeight: "600", opacity: ".7", fontSize: "14px"}}> 
                            {firstName.slice(0,1)}{lastName.slice(0,1)}
                        </div>
                        <h3> {firstName} {lastName} </h3>
                    </div>
                <div style={{display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", alignItems: "center", textAlign: "left"}}>
                    <div style={{display: 'flex', flexDirection: "column", gap: "9px", justifyContent: "flex-end"}}> 
    ...
    return(
        <div style={{display: 'grid', gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: "10px", flexWrap: "wrap", position: "relative", width: "100%"}}>
            {data ? 
