    ...
    const phaseAdminData = () => {
        return(
            <div style={{display: "flex", flexDirection: "row",  width: "100%", gap: "5px"}}>
               <div> <h4> Contract {dollarFormatter(TotalContract)} </h4> </div>
               |
               <div> <h4> COGS {dollarFormatter(TotalCost)} </h4> </div>
            </div>
        )
    }
    ...
    const phaseBody = () => {
        ...
        return(
            <div className="phase-body">
                ...
                <div className="phase-box"> <h3> P{PhaseNum} </h3>  </div>
                <div style={{display: "flex", flexDirection: "column", gap: "0px"}}> 
                    <h3> Phase {PhaseNum} </h3>
                    <h4> {phaseNumToMonth(PhaseNum)} 20{YearNum} </h4>
                </div>
                </div>
